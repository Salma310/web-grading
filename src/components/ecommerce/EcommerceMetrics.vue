<template>
  <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:gap-6">
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
      ...
      <div v-if="loading" class="mt-5 h-8 w-24 animate-pulse rounded bg-gray-200 dark:bg-gray-700" />
      <div v-else class="flex items-end justify-between mt-5">
        <div>
          <span class="text-sm text-gray-500 dark:text-gray-400">Total Petani</span>
          <h4 class="mt-2 font-bold text-gray-800 text-title-sm dark:text-white/90">
            {{ summary?.farmers?.total ?? '-' }}
          </h4>
          <span class="text-xs text-gray-400 dark:text-gray-500">{{ summary?.farmers?.active ?? 0 }} aktif</span>
        </div>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
      ...
      <div v-else class="flex items-end justify-between mt-5">
        <div>
          <span class="text-sm text-gray-500 dark:text-gray-400">Total Batch</span>
          <h4 class="mt-2 font-bold text-gray-800 text-title-sm dark:text-white/90">
            {{ summary?.batches?.total ?? '-' }}
          </h4>
          <span class="text-xs text-gray-400 dark:text-gray-500">{{ summary?.batches?.open ?? 0 }} open</span>
        </div>
        <span
          :class="[
        ...
        >
          <svg v-if="gradingGrowth >= 0" class="fill-current" width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path fill-rule="evenodd" clip-rule="evenodd" d="M5.56462 1.62393C5.70193 1.47072 5.90135 1.37432 6.12329 1.37432C6.1236 1.37432 6.12391 1.37432 6.12422 1.37432C6.31631 1.37415 6.50845 1.44731 6.65505 1.59381L9.65514 4.5918C9.94814 4.88459 9.94831 5.35947 9.65552 5.65246C9.36273 5.94546 8.88785 5.94562 8.59486 5.65283L6.87329 3.93247L6.87329 10.125C6.87329 10.5392 6.53751 10.875 6.12329 10.875C5.70908 10.875 5.37329 10.5392 5.37329 10.125L5.37329 3.93578L3.65516 5.65282C3.36218 5.94562 2.8873 5.94547 2.5945 5.65248C2.3017 5.35949 2.30185 4.88462 2.59484 4.59182L5.56462 1.62393Z" fill="" />
          </svg>
          <svg v-else class="fill-current" width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path fill-rule="evenodd" clip-rule="evenodd" d="M5.31462 10.3761C5.45194 10.5293 5.65136 10.6257 5.87329 10.6257C5.8736 10.6257 5.8739 10.6257 5.87421 10.6257C6.0663 10.6259 6.25845 10.5527 6.40505 10.4062L9.40514 7.4082C9.69814 7.11541 9.69831 6.64054 9.40552 6.34754C9.11273 6.05454 8.63785 6.05438 8.34486 6.34717L6.62329 8.06753L6.62329 1.875C6.62329 1.46079 6.28751 1.125 5.87329 1.125C5.45908 1.125 5.12329 1.46079 5.12329 1.875L5.12329 8.06422L3.40516 6.34719C3.11218 6.05439 2.6373 6.05454 2.3445 6.34752C2.3017 6.64051 2.30185 7.11538 2.59484 7.40818L5.31462 10.3761Z" fill="" />
          </svg>
          {{ Math.abs(gradingGrowth) }}%
        </span>
      </div>
    </div>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
      <div class="flex items-center justify-center w-12 h-12 bg-gray-100 rounded-xl dark:bg-gray-800">
        <svg class="fill-gray-800 dark:fill-white/90" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path fill-rule="evenodd" clip-rule="evenodd" d="M3 6C3 4.34315 4.34315 3 6 3H18C19.6569 3 21 4.34315 21 6V18C21 19.6569 19.6569 21 18 21H6C4.34315 21 3 19.6569 3 18V6ZM6 5C5.44772 5 5 5.44772 5 6V18C5 18.5523 5.44772 19 6 19H18C18.5523 19 19 18.5523 19 18V6C19 5.44772 18.5523 5 18 5H6ZM7 9C7 8.44772 7.44772 8 8 8H16C16.5523 8 17 8.44772 17 9C17 9.55228 16.5523 10 16 10H8C7.44772 10 7 9.55228 7 9ZM7 13C7 12.4477 7.44772 12 8 12H13C13.5523 12 14 12.4477 14 13C14 13.5523 13.5523 14 13 14H8C7.44772 14 7 13.5523 7 13Z" fill="" />
        </svg>
      </div>
      <div v-if="loading" class="mt-5 h-8 w-24 animate-pulse rounded bg-gray-200 dark:bg-gray-700" />
      <div v-else class="flex items-end justify-between mt-5">
        <div>
          <span class="text-sm text-gray-500 dark:text-gray-400">Grading Bulan Ini</span>
          <h4 class="mt-2 font-bold text-gray-800 text-title-sm dark:text-white/90">
            {{ summary?.gradings?.thisMonth ?? '-' }}
          </h4>
          <span class="text-xs text-gray-400 dark:text-gray-500">Total: {{ summary?.gradings?.total ?? 0 }}</span>
        </div>
        <div class="flex flex-col items-end gap-1">
          <span class="text-xs text-gray-500 dark:text-gray-400">Grade A</span>
          <span class="text-sm font-semibold text-success-600 dark:text-success-400">
            {{ summary?.gradeThisMonth?.A ?? 0 }}
          </span>
        </div>
      </div>
    </div>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
      <div class="flex items-center justify-center w-12 h-12 bg-gray-100 rounded-xl dark:bg-gray-800">
        <svg class="fill-gray-800 dark:fill-white/90" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path fill-rule="evenodd" clip-rule="evenodd" d="M2 12C2 6.47715 6.47715 2 12 2C17.5228 2 22 6.47715 22 12C22 17.5228 17.5228 22 12 22C6.47715 22 2 17.5228 2 12ZM12 4C7.58172 4 4 7.58172 4 12C4 16.4183 7.58172 20 12 20C16.4183 20 20 16.4183 20 12C20 7.58172 16.4183 4 12 4ZM11 8C11 7.44772 11.4477 7 12 7C12.5523 7 13 7.44772 13 8V12C13 12.2652 12.8946 12.5196 12.7071 12.7071L10.7071 14.7071C10.3166 15.0976 9.68342 15.0976 9.29289 14.7071C8.90237 14.3166 8.90237 13.6834 9.29289 13.2929L11 11.5858V8Z" fill="" />
        </svg>
      </div>
      <div v-if="loading" class="mt-5 h-8 w-24 animate-pulse rounded bg-gray-200 dark:bg-gray-700"/>
      <div v-else class="mt-5">
        <span class="text-sm text-gray-500 dark:text-gray-400">Distribusi Grade Bulan Ini</span>
        <div class="mt-3 flex items-center gap-2 flex-wrap">
          <span
            v-for="(grade, key) in gradeColors"
            :key="key"
            class="flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full"
            :class="grade.bg"
          >
            <span :class="grade.text">{{ key }}: {{ summary?.gradeThisMonth?.[key as keyof GradeDistribution] ?? 0 }}</span>
          </span>
        </div>
      </div>
    </div>
...
import type { DashboardSummary, GradeDistribution } from '../../types/dashboardtypes'
...
export default defineComponent({
  ...
  props: {
    summary: {
      type: Object as PropType<DashboardSummary | null>,
      default: null,
    },
    loading: {
      type: Boolean,
  ...
  setup(props) {
    const gradeColors: Record<keyof GradeDistribution, GradeColorConfig> = {
      A:      { bg: 'bg-success-50 dark:bg-success-500/15', text: 'text-success-600 dark:text-success-400' },
      B:      { bg: 'bg-blue-50 dark:bg-blue-500/15',       text: 'text-blue-600 dark:text-blue-400' },
      C:      { bg: 'bg-warning-50 dark:bg-warning-500/15', text: 'text-warning-600 dark:text-warning-400' },
      REJECT: { bg: 'bg-error-50 dark:bg-error-500/15',     text: 'text-error-600 dark:text-error-400' },
    }
    ...
    const gradingGrowth = computed((): number => props.summary?.gradings?.growth ?? 0)

    return { gradeColors, activePercent, gradingGrowth, GradeDistribution: null as unknown as GradeDistribution }
  },
})
</script>