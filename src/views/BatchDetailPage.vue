            ...
            <div class="space-y-5 px-6 py-5 max-h-[70vh] overflow-y-auto">
              ...
              <div>
                ...
                <div
                  ...
                  :class="isDragging ? 'border-brand-400 bg-brand-50 dark:border-brand-600 dark:bg-brand-900/10' : 'border-gray-200 hover:border-brand-300 dark:border-gray-700'"
              ...
              <div>
                ...
                <div class="flex gap-2">
                  <button type="button" @click="gradingMode = 'ai'; modalForm.grade = ''"
                    ...
                    :class="gradingMode === 'ai' ? 'border-brand-400 bg-brand-50 text-brand-600 dark:border-brand-600 dark:bg-brand-900/20 dark:text-brand-400' : 'border-gray-200 text-gray-500 hover:border-gray-300 dark:border-gray-700 dark:text-gray-400'">
                    Auto (AI)
                  </button>
                  <button type="button" @click="gradingMode = 'manual'"
                    class="flex-1 rounded-lg border py-2 text-sm font-medium transition-colors"
                    :class="gradingMode === 'manual' ? 'border-brand-400 bg-brand-50 text-brand-600 dark:border-brand-600 dark:bg-brand-900/20 dark:text-brand-400' : 'border-gray-200 text-gray-500 hover:border-gray-300 dark:border-gray-700 dark:text-gray-400'">
                    Manual
                  </button>
                </div>
...
const gradePercent = (count) => !batch.value?.total_fruits ? '0%' : `${Math.round((count / batch.value.total_fruits) * 100)}%`
...
// ─── Modal ────────────────────────────────────────────────
// ✅ Tidak ada guard isSubmittingGrading di sini
// Dipanggil bebas: dari tombol X, backdrop, Batal, maupun setelah sukses submit
const closeModal = () => {
...
const loadGradings = async () => {
  ...
  try { const res = await getGradingsByBatch(batchId); gradings.value = res.data }
  finally { isLoadingGradings.value = false }
}
...
const handleCloseBatch = async () => {
  ...
  try {
    ...
    await loadBatch()
    showToast('success', 'Batch berhasil ditutup', batch.value?.lot_code)
  } catch (err) {
...
/* Toast: muncul dari bawah dengan sedikit bounce */
